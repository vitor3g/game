...
export class BaseEntity implements IGameEntity {
  ...
  removeComponent<T extends IGameComponent>(
    componentType: new (...args: any[]) => T,
  ): boolean {
    const type = (componentType as any).prototype.type ?? componentType.name;
    const component = this.components.get(type);
  ...
  getScript<T extends IGameScript>(
    scriptType: new (...args: any[]) => T,
  ): T | null {
  ...
  removeScript<T extends IGameScript>(
    scriptType: new (...args: any[]) => T,
  ): boolean {
    const type = (scriptType as any).prototype.type ?? scriptType.name;
    const script = this.scripts.get(type);
  ...
  public setEntityData(key: string, value: any): void {
    this.data[key] = value;
  }

  public getEntityData(key: string): any {
