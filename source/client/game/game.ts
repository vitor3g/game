...
export class Game {
  ...
  public async start() {
    g_core.getAssetManager().register("s15", "/data/vehicles/veh.glb", AssetType.MODEL_GLTF);
  ...
  private _createTestTrack(physics: AmmoPhysics) {
    this._createFlatPlatform(physics, { x: 0, y: 0, z: 0 }, { width: 50, height: 50 }, 0x8ecccc);

    this._createRamp(physics, { x: 40, y: 0, z: 0 }, { width: 20, height: 10 }, 15, 0xcc8e8e);

    this._createFlatPlatform(physics, { x: 70, y: 5.3, z: 0 }, { width: 30, height: 20 }, 0x8ecc8e);

    this._createRamp(physics, { x: 100, y: 5.3, z: 0 }, { width: 20, height: 10 }, -15, 0xcc8ecc);

    this._createBumpyRoad(physics, { x: -50, y: 0, z: 0 }, { length: 60, width: 15 }, 6);
  ...
  private _createFlatPlatform(physics: AmmoPhysics, position: { x: number, y: number, z: number }, size: any, color = 0x8ecc8e) {
  ...
  private _createRamp(physics: AmmoPhysics, position: { x: number, y: number, z: number }, size: any, angle: number, color = 0x8ecccc) {
    const ramp = physics.add.box({
      x: position.x,
      y: position.y + (Math.sin(THREE.MathUtils.degToRad(angle)) * size.width) / 2,
      z: position.z,
      width: size.width,
      height: 2,
      depth: size.height,
  ...
  private _createBumpyRoad(physics: AmmoPhysics, position: { x: number, y: number, z: number }, size: any, bumpCount: number) {
    const bumpSpacing = size.length / bumpCount;
    const bumpHeight = 0.6;

    this._createFlatPlatform(physics,
    ...
    for (let i = 0; i < bumpCount; i++) {
      const x = position.x - size.length / 2 + (i * bumpSpacing) + bumpSpacing / 2;
  ...
  private _createSlalomCourse(physics: AmmoPhysics, position: { x: number, y: number, z: number }, length: number) {
    this._createFlatPlatform(physics,
      { x: position.x, y: position.y, z: position.z },
  ...
  private _createLoop(physics: AmmoPhysics, position: { x: number, y: number, z: number }) {
    this._createFlatPlatform(physics,
    ...
    this._createBankedCurve(physics,
  ...
  private _createBankedCurve(physics: AmmoPhysics, position: { x: number, y: number, z: number }, direction: string) {
    const segments = 8;
    const radius = 20;
    const width = 10;
    const bankAngle = 15;
